...
function correlationLabel(val) {
  ...
  if (abs >= 0.8) return { text: val > 0 ? "Strongly correlated" : "Strongly inversely correlated", tone: val > 0 ? "negative" : "positive" };
  if (abs >= 0.5) return { text: val > 0 ? "Moderately correlated" : "Moderately inversely correlated", tone: "neutral" };
...
function analyseOneTicker(ticker) {
  return Promise.all([fetchOptions(ticker), fetchRate()])
    .then(async ([optData, rateData]) => {
      ...
      const opportunity = deriveOpportunity(valuation, quality, risk, options, valuation.analystUpsidePct);
...
export default function ComparePage({ tickers = [] }) {
  ...
  const [activeTickers, setActiveTickers] = useState(
    tickers.length >= 2 ? tickers.slice(0, 2) : [],
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      analyseOneTicker(activeTickers[0]).catch((e) => ({ error: e.message, ticker: activeTickers[0] })),
      analyseOneTicker(activeTickers[1]).catch((e) => ({ error: e.message, ticker: activeTickers[1] })),
    ])
    ...
    <div className="compare-page">
      ...
      {loading && (
        <div className="compare-loading">
          ...
          <span>Analysing {activeTickers[0]} and {activeTickers[1]}&hellip;</span>
        </div>
      )}
        ...
        <>
          ...
          <div className="compare-header">
            ...
            <div className="compare-header__links">
              <a href={`/${encodeURIComponent(activeTickers[0])}`} className="compare-ticker-link">
                View {results[0]?.name || t0} &rarr;
              </a>
              <a href={`/${encodeURIComponent(activeTickers[1])}`} className="compare-ticker-link">
                ...
                <tbody>
                  {SCORE_ROWS.map(({ key, label }) => {
                    ...
                    return (
                      <tr key={key}>
                        <td>{label}</td>
                        <td className={a != null && b != null ? (a > b ? "metric-better" : a < b ? "metric-worse" : "") : ""}>
                          {a != null ? Math.round(a) : "N/A"}
                        </td>
                        <td className={a != null && b != null ? (b > a ? "metric-better" : b < a ? "metric-worse" : "") : ""}>
                ...
                <table className="data-table compare-table">
                  ...
                  <tbody>
                    {metricA.map((row, i) => {
                      ...
                      const hasBoth = va != null && Number.isFinite(va) && vb != null && Number.isFinite(vb);
                      let clsA = "", clsB = "";
                      if (hasBoth) {
                        if (row.lower) {
              ...
              <div className="terminal-card">
                <Plot
                  ...
                  layout={{
                    ...
                    xaxis: { fixedrange: true, gridcolor: c.borderLight, tickfont: { color: c.textMuted } },
                    yaxis: {
          ...
          {corrResult && (
            <div className="terminal-card compare-correlation">
              ...
              <div className="compare-correlation__row">
                <span className="compare-correlation__value">{corrVal != null ? corrVal.toFixed(2) : "N/A"}</span>
                <span className={`compare-correlation__label compare-correlation__label--${corrTone}`}>{corrText}</span>
              </div>
