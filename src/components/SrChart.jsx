...
export default function SrChart({ ticker, history, spot, sr, entryInfo, overlays = {} }) {
  const { data, layout } = useMemo(() => {
    ...
    for (const level of [...supports, ...resistances]) {
      ...
      shapes.push({
        ...
        line: { color: colour, width: 2, dash }, opacity: 0.55,
      });
    ...
    for (const level of [...supports, ...resistances]) {
      ...
      annotations.push({ text: `<b>${label}</b>`, x: 0.0, xref: "paper", y: level.price, yref: "y", showarrow: false, xanchor: "left", yshift: 14, font: { size: 12, color: clr }, ...tag });
    }
    ...
    if (entry != null) {
      ...
      shapes.push({
        type: "rect", x0: 0, x1: 1, xref: "paper", y0: entry - bandH, y1: entry + bandH,
        fillcolor: entryFill, line: { width: 0 }, layer: "above",
      });
      annotations.push({ text: `<b>Entry</b> $${entry.toFixed(0)}`, x: 1.0, xref: "paper", y: entry, yref: "y", showarrow: false, xanchor: "right", yshift: -14, font: { size: 12, color: COLORS.accentWarm }, ...tag });
    }

    if (stop != null) {
      shapes.push({
        type: "line", x0: 0, x1: 1, xref: "paper", y0: stop, y1: stop,
        line: { color: COLORS.red, width: 2.5, dash: "dot" }, opacity: 0.7,
      });
      annotations.push({ text: `<b>Stop</b> $${stop.toFixed(0)}`, x: 1.0, xref: "paper", y: stop, yref: "y", showarrow: false, xanchor: "right", yshift: -14, font: { size: 12, color: COLORS.red }, ...tag });
    }

    if (target != null) {
      shapes.push({
        type: "line", x0: 0, x1: 1, xref: "paper", y0: target, y1: target,
        line: { color: COLORS.green, width: 2.5, dash: "dot" }, opacity: 0.7,
      });
      annotations.push({ text: `<b>Target</b> $${target.toFixed(0)}`, x: 1.0, xref: "paper", y: target, yref: "y", showarrow: false, xanchor: "right", yshift: 14, font: { size: 12, color: COLORS.green }, ...tag });
    }

    const lo = {
      ...LAYOUT_DEFAULTS,
      margin: { l: 65, r: 40, t: 50, b: 50 },
      title: { text: `<b>${ticker}</b>  \u2014  Potential Entry Setup`, font: { size: 16, color: COLORS.text }, x: 0.01 },
      xaxis: { ...axisStyle(), title: "", rangeslider: { visible: false } },
      yaxis: { ...axisStyle(), title: "Price ($)", tickprefix: "$", autorange: true },
      showlegend: true,
      legend: { bgcolor: "rgba(247,245,240,0.85)", bordercolor: COLORS.borderLight, borderwidth: 1, font: { size: 12 }, x: 0.01, y: 0.99, xanchor: "left", yanchor: "top" },
  ...
  return (
    <div className="chart-section">
      <Plot data={data} layout={layout} config={PLOTLY_CONFIG} useResizeHandler style={{ width: "100%", height: "100%" }} />
    </div>
  );
}
